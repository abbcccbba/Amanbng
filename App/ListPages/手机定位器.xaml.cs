...
namespace App.ListPages
{
    ...
    public sealed partial class 手机定位器 : Page
    {
        ...
        public 手机定位器()
        {
            ...
            N4_ShowOpen.Visibility = Visibility.Collapsed; // 设置默认结果标签为隐藏
            MainPic.Visibility = Visibility.Collapsed; // 这是隐藏图片框
        }

        private async void N4_start(object sender, RoutedEventArgs e)
        {
            ...
            N4_ShowOpen.Visibility = Visibility.Collapsed;
            MainPic.Visibility = Visibility.Collapsed;
            N4_ShowOpen.Text = "正在定位"; // 打印初始文字，告诉用户信息
            ...
            DateTime startTime;
            N4_ShowOpen.Visibility = Visibility.Visible; 
            N4_ring.IsActive = true; 
            ...
            N4_ring.IsActive = false; // 结束等待
 
            N4_ShowOpen.Foreground = new SolidColorBrush(Windows.UI.Color.FromArgb(255, 10, 213, 9)); // 改个颜色，告诉用户已完成
            /* 接下来一些简单的判断这个函数就结束了  */
            if (checkBox.IsChecked == true)
            {
                MainPic.Visibility = Visibility.Visible;
                N4_ShowOpen.Text = "已查找到 在：";
            }
            else N4_ShowOpen.Text = "已查找到 在：" + textBox.Text;
        }
    }
}
